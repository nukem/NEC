      ...
      <form action=".?id=<?= $id ?>" method="post" enctype="multipart/form-data" > 
        ...
        <div class="right-col-padding1"> 
          <div class="width-99pct"> 
			...
			<tr><td colspan="4"><label>Accessibility &bull;</label></td></tr>
				<?php 
				$count = 0;
				foreach($permissions as $perm){
					if(!($count % 4)){ echo '<tr>'."\n"; }
					echo '<td><input type="checkbox" name="permissions[]" value="'.$perm['category_id'].'"';
					if(in_array($perm['category_id'], $accesseses)){ echo 'checked="checked"'; }
					echo '/>'.$perm['category_name'].'</td>'."\n";
					if(!(($count + 1) % 4) && $count  != 0){ echo '</tr>'."\n"; }
					$count++;
				}
				if($count % 4){ echo '</tr>'; }
				*/
