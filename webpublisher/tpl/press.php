            ...
            <table class="rec-table"> 
              <? require ("tpl/inc/record.php"); ?> 
				<tr>
                    ...
                    <input type="text" name="press_date" class="textfield width-100pct date-pick" value="<? if (isset($_POST['title'])) echo htmlentities($_POST['press_date'], ENT_QUOTES); else echo date('d M Y', strtotime($record['press_date']));  ?>" />
                    </td>
                    <td colspan="2">&nbsp;</td>
                </tr>
              <tr> 
...
$(document).ready(function(){
	...
	/*
	if($.browser.msie){
		$('.date-pick').datePicker({startDate:'2007-01-01'}).dpSetOffset(0, 0);
	}else{
		$('.date-pick').datePicker({startDate:'2007-01-01'});
	}
	*/
