          ...
          <div class="width-99pct"> 
            <table class="rec-table"> 
              ...
              <? if (is_file ($cfg['data'] . $id . "." . $record['extension'])) { ?> 
                  <img src="img/ico-file/<? if (is_file ("img/ico-file/{$record['extension']}.gif")) echo "hires_image.gif"; else echo "hires_image.gif"; ?>" alt="Preview" width="16" height="16" /> <a href="file-preview.php?file=<?=  $id . '.' . $record['extension'] ?>&amp;filename=<?= $record['uri'] . "." . $record['extension'] ?>"><?= $record['uri'] . "." . $record['extension'] ?></a>
              <? } else { ?>
                  <?= $lang[68] ?>
              <? } ?>
			    </td>
